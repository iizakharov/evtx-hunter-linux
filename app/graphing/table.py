...
def create_interesting_events_table(rule_dict):
    ...
    for event in utils.retrieve_all_events():
        item_event_channel = event['data']["Event"]["System"]["Channel"]
        ...
        for filter_field in rule_dict["event_filter"].keys():
            if filter_field not in event["data"]["Event"]["EventData"].keys():
                event_matches_filter = False
                break
            elif str(event["data"]["Event"]["EventData"][filter_field]) != str(rule_dict["event_filter"][filter_field]):
                event_matches_filter = False
                break
            else:
        ...
        if event_matches_filter:
            ...
            time_string = event["data"]["Event"]["System"]["TimeCreated"]["#attributes"]["SystemTime"]
            event_date = datetime.fromisoformat(time_string.replace('Z', '+00:00'))

            graph_data_dict["timestamp"].append(event_date)

            if rule_dict["log_full_event"]:
                graph_data_dict["full_event"].append(json.dumps(event["data"]["Event"], indent=4, sort_keys=True))
...
def create_first_occurence_table(rule_dict):
    ...
    for event in utils.retrieve_all_events():
        ...
        if event_matches_filter:
            ...
            if rule_dict["monitored_field"] in event["data"]["Event"]["EventData"].keys():
                if event["data"]["Event"]["EventData"][rule_dict["monitored_field"]] \
                        not in graph_data_dict[rule_dict["monitored_field"]]:
                    graph_data_dict[rule_dict["monitored_field"]].append(
                        event["data"]["Event"]["EventData"][rule_dict["monitored_field"]])
                    ...
                    if event_date < earlier_seen_date:
                        ...
                        if rule_dict["log_full_event"]:
                            graph_data_dict["full_event"][first_occurence_index] = json.dumps(event["data"]["Event"], indent=4, sort_keys=True)
